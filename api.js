...
class ApiService {
  ...
  async updateScheduleName(scheduleIndex, newName) {
    if (!this.isAuthenticated()) return { success: false };

    try {
      const response = await fetch(`${getApiBaseUrl()}/schedules/${scheduleIndex}/name`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': this.token,
      ...
      if (!response.ok) {
        ...
        return { success: false, error: errorData.msg || 'Error al actualizar el nombre del horario' };
      }

      const updatedScheduleData = await response.json();
      return { success: true, scheduleData: updatedScheduleData };
    } catch (error) {
      console.error('Update schedule name error:', error);
      return { success: false, error: error.message };
    }
  }
